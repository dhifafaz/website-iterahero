...
import { getActuatorDetail,addAutomation,monitoringApi,apiGetAutomation } from '../../Utility/api_link'
...
const AutomationEdit = () => {
  ...
  const submit = (id_actuator, id_sensor,condition,status_lifecycle,constanta) => {
    ...
    data.constanta= constanta;
      ...
      if (data.id_actuator == "" || data.id_sensor == "" || data.condition == "" || data.status_lifecycle == "" || data.constanta=="") {
          return (alert('Masih ada yang belum di isi'))
      }
      else {
          setIsLoading(false)
          updateAutomation(id_actuator, id_sensor,condition,status_lifecycle,constanta);
      }
  }
  const header = localStorage.getItem('token')

  const updateAutomation = (valueActuator, valueSensor, valueCondition, valueStatus_lifecycle, valueConstanta) => {
      axios.put(
          `${apiGetAutomation}${id}`,
          {
  ...
  const getAutomation = async() => {
    setIsLoading(true)
    await axios.get(`${apiGetAutomation}${id}`, {
      headers: {
          'Authorization': 'Bearer ' + header
        }
      })
  ...
  const getActuator = async() => {
    setIsLoading(true)
    await axios.get(`${getActuatorDetail}${id}`, {
      headers: {
          'Authorization': 'Bearer ' + header
        }
      })
  ...
  const getSensor = async(id_greenhouse) =>{
    setIsLoading(true)
    await axios.get(`${monitoringApi}${id_greenhouse}`, {
          headers: {
              'Authorization': 'Bearer ' + header
            }
          })
            ...
            >
                {({
                    ...
                    <Form>
                      <FormControl
                        ...
                        <Select
                          ...
                          name="actuator"
                          id="actuator">
                          <option value={dataAutomation.actuator.id} color={"var(--color-primer)"}>
                      ...
                      <FormControl
                        ...
                        <FormLabel color={"var(--color-primer)"}>Kondisi Automatis</FormLabel>
                        <Select
                          color={"var(--color-primer)"}
                          onBlur={handleBlur}
                          onChange={handleChange}
                    ...
                    <FormControl
                      ...
                      <FormLabel color={"var(--color-primer)"}>Status Lifecycle</FormLabel>
                      <Select
                        color={"var(--color-primer)"}
                        onBlur={handleBlur}
                        onChange={handleChange}
                     ...
                     <FormControl
                      ...
                      <Button
                          ...
                          type="submit"
                          onClick={() => submit(values.actuator,values.sensor,values.condition,values.lifecycle,values.constanta)}
                      >
                          <Text
                              fontWeight='bold'
